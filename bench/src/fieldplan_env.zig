//! FieldPlan-based env parser rewrite (user-provided).
//! Comptime walks the struct once into leaf/alloc descriptors, then fills
//! nested pointers and leaf fields in two flat passes.
...
fn fieldInfoByName(comptime T: type, comptime name: []const u8) std.builtin.Type.StructField {
    inline for (@typeInfo(T).@"struct".fields) |field| {
        if (comptime std.mem.eql(u8, field.name, name)) return field;
    }
    @compileError("field not found: " ++ name);
}
...
fn convertScalar(comptime T: type, allocator: std.mem.Allocator, value: []const u8) !T {
    return switch (@typeInfo(T)) {
        .int => std.fmt.parseInt(T, value, 10) catch ParseError.TypeConversionError,
        .float => std.fmt.parseFloat(T, value) catch ParseError.TypeConversionError,
        .bool => blk: {
...
fn convertValue(comptime FieldType: type, allocator: std.mem.Allocator, value: []const u8) !FieldType {
...
fn duplicateScalar(comptime T: type, allocator: std.mem.Allocator, value: T) !T {
    return switch (@typeInfo(T)) {
        .pointer => |ptr| blk: {
            if (ptr.size == .slice and ptr.child == u8) {
                break :blk try allocator.dupe(u8, value);
...
fn duplicateIfNeeded(comptime FieldType: type, allocator: std.mem.Allocator, value: FieldType) !FieldType {
...
fn freeScalar(comptime T: type, allocator: std.mem.Allocator, value: T) void {
    switch (@typeInfo(T)) {
        .pointer => |ptr| {
            if (ptr.size == .slice and ptr.child == u8) {
...
fn freeLeaf(comptime FieldType: type, allocator: std.mem.Allocator, value: FieldType) void {
    if (@typeInfo(FieldType) == .optional) {
        if (value) |inner| {
...
pub fn parse(
    ...
    errdefer freePartial(T, allocator, container, nested_allocated, leaves_initialized);
...
fn free(comptime T: type, allocator: std.mem.Allocator, container: *T) void {
    @setEvalBranchQuota(100_000);
    const Plan = FieldPlan(T);
