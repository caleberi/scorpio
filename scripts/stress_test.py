...
class StressClient:
    ...
    def _pick(self) -> tuple[str, str, str]:
        roll = random.random()
        acc = 0.0
        kind = DEFAULT_MIX[-1][0]
        for name, weight in self.config.mix:
            acc += weight
            if roll <= acc:
                kind = name
                break
    ...
    def run(self) -> dict:
        ...
        return {
            ...
            "by_phase": {k: summarize(v) for k, v in by_phase.items()},
...
def render_report(data: dict) -> str:
    ...
    phase_p95 = [data["by_phase"][p]["latency_ms"]["p95"] or 0 for p in phases]
...
<main>
  ...
  <ul>
    ...
    <li>This is a client-side observation from one region; it includes internet RTT to Render.</li>
  </ul>
</main>
</body>
</html>
...
def phases_for(profile: str) -> list[Phase]:
    if profile == "quick":
        return [
            Phase("c2", 2, 8),
            Phase("c6", 6, 10),
