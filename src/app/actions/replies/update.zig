...
pub const Update = struct {
    pub const friendly_name = "UpdateReply";
    pub const description = "Update a reply on a blog comment";
    ...
    pub fn run(inputs: Inputs, exits: *action.Exits(@This())) !void {
        ...
        const updated = app.db.updateReply(
            exits.allocator,
            blog_id,
            inputs.comment_id,
            inputs.reply_id,
            ...
            return exits.send(.error_, .{ .error_message = "failed to update reply" });
        } orelse {
            return exits.send(.notFound, .{ .error_message = "reply not found" });
        };
        defer db_mod.freeReply(exits.allocator, updated);
