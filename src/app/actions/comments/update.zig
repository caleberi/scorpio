...
pub const Update = struct {
    pub const friendly_name = "UpdateComment";
    pub const description = "Update a comment on a blog post";
    ...
    pub fn run(inputs: Inputs, exits: *action.Exits(@This())) !void {
        ...
        const updated = app.db.updateComment(exits.allocator, blog_id, inputs.comment_id, inputs.body) catch {
            return exits.send(.error_, .{ .error_message = "failed to update comment" });
        } orelse {
            return exits.send(.notFound, .{ .error_message = "comment not found" });
        };
        defer db_mod.freeComment(exits.allocator, updated);
        return exits.send(.success, updated);
    }
};
